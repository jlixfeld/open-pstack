...
[Poteto](https://x.com/poteto)'s [pstack](https://github.com/cursor/plugins/tree/main/pstack), adapted to run in Claude Code and Codex without Cursor. One shared skill tree serves both harnesses. The experimental active matrix contains OpenAI Astra, Sol, Terra, and Luna; the runner retains dormant legacy provider acceptance for existing immutable lanes and receipts, including Claude Opus 5.5. Version 1.4.2-E1 is synced to Cursor pstack v0.15.1 at `f8abeddd1862dc73704e3d719dd73df0d51b8c71`. See [UPSTREAM.md](../UPSTREAM.md) for the exact sync contract.
...
│   └── agents/                       # generic Claude subagents; active OpenAI routing uses Codex descriptors
...
- **Tool and built-in mapping.** Claude tool names and built-in skills resolve through [`codex-tools.md`](../plugins/pstack/skills/poteto-mode/references/codex-tools.md). Model execution resolves separately through [`provider-dispatch.md`](../plugins/pstack/skills/poteto-mode/references/provider-dispatch.md). The active matrix runs Astra, Sol, Terra, and Luna through Codex.
...
- **Models.** `/setup-pstack` writes provider-qualified per-role descriptors. The experimental active matrix contains Astra, Sol, Terra, and Luna. Astra's documented portable efforts run from `low` through `max`, excluding `ultra`; Sol and Terra also allow `ultra`. The initial map separates Terra-high feature work from Luna-high refactoring. It uses medium effort for broad work and independent Astra/Sol review lanes; high effort is an explicit choice for coupled retry, persisted state, authentication, or concurrency work; xhigh is reserved for the hardest tasks; max has no default. Arena, Architect, How critics, and Interrogate run every stored lane in order. The cross-judge pool contains Astra and Sol and chooses the model different from the likely base when possible. The stored list length is the only candidate or reviewer count, and consensus requires multiple completed lanes. A rerun preserves exact role lanes and probes only unique final-map descriptors. In Codex, active descriptors use native `spawn_agent`; in Claude Code, they use the deterministic Codex runner. Immutable legacy Claude and Grok routes remain accepted by the runner but are not active selections. `inherit-parent` and `auto` require a Codex OpenAI parent for OpenAI-only execution. An explicit pstack request or confirmation of the disclosed model sheet authorizes every selected native or external lane to receive the assigned source code and task context. Automatic SessionStart routing does not grant that authorization. Children never detect the parent or reroute themselves.
...
- **`gh` CLI** — default system-level forge for the Babysit, Shipping, and stack playbooks. Install via [`brew install gh`](https://cli.github.com) and authenticate with `gh auth login`. When the Origin CLI is installed and can resolve the repository, those playbooks prefer it instead.
...
- **Claude Code and Codex CLIs** — the active matrix needs Codex. The external runner still recognizes immutable legacy provider routes and their receipt formats. Same-provider work stays native; the runner refuses it.
- **`gt` (Graphite CLI)** — only for Orchestrate and its frontier tooling. The Babysit, Shipping, and autopilot playbooks use the resolved forge and do not require it.
...
The active matrix ships no pinned Claude frontier agents. Generic `poteto-agent` and `comment-sicko` remain available. Immutable legacy runner support remains deliberately separate from active selectable model rows.
...
| Configured panels (arena, architect, interrogate, how-critics) | Provider dispatch preserves each role's ordered lanes. Initial Arena, How critics, and Interrogate panels use `codex:gpt-6-astra@medium`, `codex:gpt-5.6-sol@medium`; Architect retains `codex:gpt-6-astra@high`, `codex:gpt-5.6-sol@high`. The list length is the sole fan-out count, and consensus requires multiple completed lanes. Terra high remains the feature-implementation model. Same-provider lanes stay native. |

### Cross-vendor dispatch

The active experiment uses Codex OpenAI routing without adding a daemon or model-router service. The top-level parent chooses every route and each external process receives a complete task directly, so there is no supervising model invocation and no child-side harness detection. The runner's legacy provider handling remains so existing immutable lanes and receipts are still understood.
