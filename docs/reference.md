...
- **Tool and built-in mapping.** Claude tool names and built-in skills resolve through [`codex-tools.md`](../plugins/pstack/skills/poteto-mode/references/codex-tools.md). Model execution resolves separately through [`provider-dispatch.md`](../plugins/pstack/skills/poteto-mode/references/provider-dispatch.md). The active matrix runs GPT-6 Astra, Sol, and Luna through Codex.
...
- **Models.** `/setup-pstack` writes provider-qualified per-role descriptors. The active matrix contains GPT-6 Astra, Sol, and Luna, each with efforts from `low` through `max`; pstack does not expose Sol and Luna's `none` effort, and no active family accepts `ultra`. The initial map separates Sol-high feature work from Luna-high refactoring. It uses medium effort for broad work and independent Astra/Sol review lanes; high effort is an explicit choice for coupled retry, persisted state, authentication, or concurrency work; xhigh is reserved for the hardest tasks; max has no default. Arena, Architect, How critics, and Interrogate run every stored lane in order. The cross-judge pool contains Astra and Sol and chooses the model different from the likely base when possible. The stored list length is the only candidate or reviewer count, and consensus requires multiple completed lanes. A rerun preserves exact role lanes and probes only unique final-map descriptors. In Codex, active descriptors use native `spawn_agent`; in Claude Code, they use the deterministic Codex runner. Immutable Claude and Grok routes remain accepted by the runner, including Claude Opus 5.5, but are not active selections. `inherit-parent` and `auto` require a Codex OpenAI parent for OpenAI-only execution. An explicit pstack request or confirmation of the disclosed model sheet authorizes every selected native or external lane to receive the assigned source code and task context. Automatic SessionStart routing does not grant that authorization. Children never detect the parent or reroute themselves.
...
| Configured panels (arena, architect, interrogate, how-critics) | Provider dispatch preserves each role's ordered lanes. Initial Arena, How critics, and Interrogate panels use `codex:gpt-6-astra@medium`, `codex:gpt-6-sol@medium`; Architect uses `codex:gpt-6-astra@high`, `codex:gpt-6-sol@high`. The list length is the sole fan-out count, and consensus requires multiple completed lanes. Sol high is the feature-implementation model. Same-provider lanes stay native. |
