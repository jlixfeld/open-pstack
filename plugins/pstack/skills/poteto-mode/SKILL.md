...
These skills share one tree across Claude Code and Codex. Read [`references/provider-dispatch.md`](references/provider-dispatch.md) whenever a configured role launches. It defines the standing authorization boundary, provider-qualified model descriptors, native/external route table, launcher, isolation, receipts, and dropout policy. Children never choose routes. When a skill names a Claude tool or built-in skill (`run`, `verify`, `plugin-dev:skill-development`), read [`references/codex-tools.md`](references/codex-tools.md) for the Codex equivalent.
...
- Any code change → complete the [changed branch coverage review](../principle-prove-it-works/references/changed-branch-coverage.md). Use existing tooling only. Inspect every uncovered changed branch and report gaps or an unavailable result with substitute verification.
...
- **Attack the Premise** (**principle-attack-the-premise**). Two or more fixes sharing one premise fail the same gate. Question that premise.
...
- **Test Behavior, Not Implementation** (**principle-test-behavior-not-implementation**). Writing, changing, or keeping a test. Assert observable behavior or a contract and check that a relevant defect fails.
...
- **Every claim carries its evidence or label in the same sentence.** Label an unmeasured statement as measured, inferred, or guess. Never hand the user a check you can run.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run bottom-up through `gh` by default or Origin when available. `playbooks/shipping.md`.
