...
Claude provider pauses wake no sooner than 30 minutes after observation. `--interval-seconds` may set a longer immutable registration policy, never a shorter one. Duplicate wakes and session restarts replay the same unreserved plan; the runner reservation permits one provider child. Once any output or receipt reservation exists, automatic relaunch stops. Plain `lane tick` writes held claims as `STALLED` diagnostics, and JSON includes them in `stalledLanes`. A diagnostic is not proof that a child died and does not release the provider. A coordinator may release only a known-dead exact claim with `orch lane release <lane-id> --attempt <attempt-id> --reason <authoritative evidence>`, then call `orch --json lane retry <lane-id>` directly for a fresh attempt. Retry reconciles under the store lock, so do not tick first and risk letting a sibling claim the provider. Terminal reconciliation units cannot retry. There is no TTL or inferred timeout. Schedulers never pass the global `--force` option; forced lock theft is an operator recovery action.

Before changing a unit's state, `orch lane check --unit <unit-id>` must exit ready. It exits 3 and names every blocking lane while any bound lane is unstarted, paused, failed, interrupted, or has missing/tampered prompt, receipt, or output evidence. `orch unit set` enforces the same gate for advancing state changes. It allows same-state metadata updates and the explicit terminal reconciliation states `abandoned` and `zombie-reconciled`; neither exception can advance a unit to `done` or another success state.
...
- `.orch.lock` is the transient single-writer PID lock. `.orch.lock.recovery.sqlite` is the persistent SQLite file whose process-held transaction serializes stale-lock recovery; its presence does not mean recovery is active.
- `.orch.lane-registry.initialized` records that the lane registry exists. `provider-lanes/registry.json` stores immutable lane specifications and attempt history. Each `provider-lanes/<lane-id>/` directory holds its prompt snapshot and any per-attempt output or receipt files.
...
2. **Install the runtime.** Run the idempotent `orch init` for new and existing stores. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `orch frontier set --repo <repo-dir>`.
...
- After a session restart: run the idempotent `orch init` against the existing store before any other store command. In-flight subagents are dead; pushed branches, open PRs, and the store are not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach in-flight work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone. Concurrent recovery losers may report `store lock recovery is held` while the winning command finishes. Treat that as transient contention and rerun after the winner exits. Do not pass `--force` or delete `.orch.lock.recovery.sqlite` for this case.
