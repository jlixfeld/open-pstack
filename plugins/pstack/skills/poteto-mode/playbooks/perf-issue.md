...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured perf-issue descriptor (default `codex:gpt-6-astra@high`) with `isolated-write` in a dedicated worktree; review the diff. Capture a post-fix trace.
