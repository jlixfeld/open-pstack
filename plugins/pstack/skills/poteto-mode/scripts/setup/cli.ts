...
export function canonicalTarget(
  ...
  home: string = homedir()
): { readonly path: string; readonly aliases: readonly string[] } {
  const expanded = value === "~"
    ? home
    : value.startsWith(`~${sep}`)
      ? resolve(home, value.slice(2))
      : resolve(cwd, value);
  const aliases = new Set<string>([expanded, value]);
  const homePrefix = `${resolve(home)}${sep}`;
  if (expanded.startsWith(homePrefix)) aliases.add(`~${sep}${expanded.slice(homePrefix.length)}`);
  return { path: expanded, aliases: [...aliases] };
}
...
function planFromUnknown(value: unknown): SetupPlan {
  ...
  if (!Array.isArray(value.edits) || !Array.isArray(value.probes) || !Array.isArray(value.sheetAliases)) throw new Error("invalid setup plan collections");
  ...
  return {
    ...
    sheetAliases: value.sheetAliases.map((alias) => string(alias, "plan sheet alias")),
    edits,
...
function verifyBaselines(plan: SetupPlan, manifestMarkdown: Uint8Array, targets: readonly [Snapshot, Snapshot]): void {
  ...
  for (const [index, item] of plan.targets.entries()) {
    if (snapshotHash(targets[index]) !== item.hash) throw new Error(`stale setup plan: target changed: ${item.path}`);
  }
}

function prepareFromPlan(plan: SetupPlan, manifestMarkdown: string, targets: readonly [Snapshot, Snapshot]) {
  const edits = parseEdits(plan.edits.map((edit) => `${edit.role}=${edit.lanes.join(",")}`), manifestMarkdown);
  const prepared = prepareSetup({
    parent: plan.parent,
    manifestMarkdown,
    sheet: targets[0],
    sheetAliases: plan.sheetAliases,
    integration: targets[1],
    edits,
  });
  const probes = prepared.probes.map(renderLane);
...
export function prepare(
  ...
  const prepared = prepareSetup({ parent: parentValue, manifestMarkdown, sheet, sheetAliases: sheetTarget.aliases, integration, edits });
  const plan: SetupPlan = {
    ...
    sheetAliases: sheetTarget.aliases,
    edits: edits.map((edit) => ({ role: edit.role, lanes: edit.lanes.map(renderLane) })),
    probes: prepared.probes.map(renderLane),
  };
...
export function commit(argv: readonly string[], capture: (path: string) => Snapshot = snapshot): void {
  const parsed = options(argv);
  known(parsed, ["plan", "probe-results"]);
  const planPath = canonicalTarget(required(parsed, "plan")).path;
  const receiptsPath = canonicalTarget(required(parsed, "probe-results")).path;
  const plan = loadPlan(planPath);
  const manifestBytes = readFileSync(plan.manifest.path);
  const targetSnapshots: [Snapshot, Snapshot] = [capture(plan.targets[0].path), capture(plan.targets[1].path)];
  verifyBaselines(plan, manifestBytes, targetSnapshots);
  const prepared = prepareFromPlan(plan, new TextDecoder().decode(manifestBytes), targetSnapshots);
