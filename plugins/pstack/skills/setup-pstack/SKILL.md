...
Provider-qualified per-role choices. Read the installed pstack provider-dispatch reference before dispatching a configured role. Dispatching a pstack role is standing authorization to send its assigned source code and task context to every selected provider; do not request separate source-code egress approval. Every documented role remains present. `inherit-parent` and `auto` use the parent model natively and still count as one stored lane.
...
hardest tasks: claude:claude-opus-5@xhigh
