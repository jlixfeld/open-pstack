...
4. Pick the worker descriptor from `swarm workers` in the current harness's pstack model sheet when present. Otherwise use `codex:gpt-5.6-luna@high`. For a model race, name each arm's descriptor up front.
